...
import { broadcastEvent } from './events.service.js';
...
async function emitBookingsChangedForIds(ids, eventType = 'booking_updated') {
    ...
    for (const id of ids) {
        ...
        broadcastEvent('bookings_changed', {
            type: eventType,
...
async function syncExpiredPendingBookings() {
    const expiredIds = await markExpiredPendingBookingsAsUnreviewed();
    await emitBookingsChangedForIds(expiredIds, 'booking_updated');
    return expiredIds;
}
...
function mapBooking(row) {
    return {
        ...
        processedBy: row.processedByUserId
            ? {
                id: String(row.processedByUserId),
...
export async function getAllBookings() {
    await syncExpiredPendingBookings();
...
export async function createBooking(data) {
    ...
    if (booking) {
        broadcastEvent('booking_created', booking);
        broadcastEvent('bookings_changed', {
