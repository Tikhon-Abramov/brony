...
import { getBookingStatusLabel, TIME_SLOTS, timeToMinutes } from '../../utils/booking';
...
export default function ScheduleSection() {
    ...
    const targetSlot = useMemo(() => {
        ...
        const targetMinutes = Math.max(timeToMinutes('09:00'), currentMinutes - 60);

        let slot = TIME_SLOTS[0];

        for (let i = 0; i < TIME_SLOTS.length; i += 1) {
            const slotMinutes = timeToMinutes(TIME_SLOTS[i]);
    ...
    useLayoutEffect(() => {
        const container = gridScrollerRef.current;
        if (!container) return;
        ...
        const tryScroll = () => {
            ...
            const left = Math.max(0, targetEl.offsetLeft - targetEl.offsetWidth);
    ...
    }, [targetSlot, bookingsForSelectedDate.length]);
        ...
        <SectionCard>
            <SectionHeader>
                <div>
                    ...
                    <Subtitle>
                        Показаны брони на текущий день. Наведи на бронь, чтобы увидеть подробности.
                    </Subtitle>
                </div>
            </SectionHeader>

            <GridScroller ref={gridScrollerRef}>
...
const StatusBadge = styled.span<{ $status: 'pending' | 'approved' | 'rejected' | 'unreviewed' }>`
    font-size: 12px;
    border-radius: 999px;
    padding: 6px 10px;
    white-space: nowrap;
